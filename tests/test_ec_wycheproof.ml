...
module Asn = struct
  let parse_point curve s =
    ...
    | Ok (((oid1, oid2), data), rest) ->
        ...
        else if not (Asn.OID.equal oid2 prime_oid) then
          Error "ASN1: wrong oid 2"
        else Ok data

  let parse_signature cs =
    let asn =
      Asn.S.(sequence2 (required unsigned_integer) (required unsigned_integer))
    in
    match Asn.(decode (codec der asn) cs) with
    | Error _ -> Error "ASN1 parse error"
    | Ok (r_s, rest) ->
        if String.length rest <> 0 then Error "ASN1 leftover" else Ok r_s
end
...
let pad ~total_len buf =
  ...
  | n when n < 0 ->
      ...
      if !is_zero then Ok (String.sub buf (abs n) total_len)
      else Error "input is too long"
  | pad_len -> Ok (String.make pad_len '\000' ^ buf)
...
type test = { public_key : string; raw_private_key : string; expected : string }

let perform_key_exchange curve ~public_key ~raw_private_key =
  to_string_result ~pp_error
    (match curve with
    | "secp256r1" -> (
        match P256.Dh.secret_of_octets raw_private_key with
        | Ok (p, _) -> P256.Dh.key_exchange p public_key
        | Error _ -> assert false)
    | "secp384r1" -> (
        match P384.Dh.secret_of_octets raw_private_key with
        | Ok (p, _) -> P384.Dh.key_exchange p public_key
        | Error _ -> assert false)
    | "secp521r1" -> (
        match P521.Dh.secret_of_octets raw_private_key with
        | Ok (p, _) -> P521.Dh.key_exchange p public_key
        | Error _ -> assert false)
    | "secp256k1" -> (
        match P256k1.Dh.secret_of_octets raw_private_key with
        | Ok (p, _) -> P256k1.Dh.key_exchange p public_key
        | Error _ -> assert false)
    | "brainpoolP256r1" -> (
        match BrainpoolP256.Dh.secret_of_octets raw_private_key with
        | Ok (p, _) -> BrainpoolP256.Dh.key_exchange p public_key
        | Error _ -> assert false)
    | "brainpoolP384r1" -> (
        match BrainpoolP384.Dh.secret_of_octets raw_private_key with
        | Ok (p, _) -> BrainpoolP384.Dh.key_exchange p public_key
        | Error _ -> assert false)
    | "brainpoolP512r1" -> (
        match BrainpoolP512.Dh.secret_of_octets raw_private_key with
...
let make_ecdsa_test curve key hash (tst : dsa_test) =
  ...
  let verified (r, s) =
    match curve with
    | "secp256r1" -> (
        match P256.Dsa.pub_of_octets key with
        | Ok key -> P256.Dsa.verify ~key (r, s) msg
        | Error _ -> assert false)
    | "secp384r1" -> (
        match P384.Dsa.pub_of_octets key with
    ...
    | "secp521r1" -> (
        ...
        | Error _ -> assert false)
    | "secp256k1" -> (
        match P256k1.Dsa.pub_of_octets key with
        | Ok key -> P256k1.Dsa.verify ~key (r, s) msg
        | Error _ -> assert false)
    | "brainpoolP256r1" -> (
        match BrainpoolP256.Dsa.pub_of_octets key with
        | Ok key -> BrainpoolP256.Dsa.verify ~key (r, s) msg
        | Error _ -> assert false)
    | "brainpoolP384r1" -> (
        match BrainpoolP384.Dsa.pub_of_octets key with
        | Ok key -> BrainpoolP384.Dsa.verify ~key (r, s) msg
        | Error _ -> assert false)
    | "brainpoolP512r1" -> (
        match BrainpoolP512.Dsa.pub_of_octets key with
...
let to_ecdsa_tests (x : ecdsa_test_group) =
  List.map (make_ecdsa_test x.key.curve x.key.uncompressed x.sha) x.tests
...
let () =
  Alcotest.run "Wycheproof tests"
    [
      ...
      ( "ECDH BrainpoolP256r1 test vectors",
        ecdh_tests "ecdh_brainpoolp256r1_test.json" );
      ( "ECDSA BrainpoolP256r1 test vectors (SHA256)",
        ecdsa_tests "ecdsa_brainpoolP256r1_sha256_test.json" );
      ( "ECDH BrainpoolP384r1 test vectors",
        ecdh_tests "ecdh_brainpoolp384r1_test.json" );
      ( "ECDSA BrainpoolP384r1 test vectors (SHA384)",
        ecdsa_tests "ecdsa_brainpoolP384r1_sha384_test.json" );
      ( "ECDH BrainpoolP512r1 test vectors",
        ecdh_tests "ecdh_brainpoolp512r1_test.json" );
