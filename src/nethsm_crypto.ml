...
module Make_field_element (P : Parameters) (F : Foreign_mont) : Field_element =
struct
  ...
  let to_montgomery x =
    F.to_montgomery x (of_fe_out x);
    x

  let from_be_octets = from_be_octets ~to_montgomery
end

module Make_field_element_usol (P : Parameters) (F : Foreign_field) :
...
module Make_point_base
  ...
  let add_ax =
    if String.length P.a = 0 then fun ~x:_ n -> n
    else
  ...
  let validate_finite_point_fe x y =
    if is_solution_to_curve_equation ~x ~y then Ok (make x y)
  ...
  let to_affine p =
    Option.map
      (fun (x, y) -> (Fe.to_octets x, Fe.to_octets y))
      (to_affine_raw p)
  ...
  let to_octets ~compress p =
    ...
    if compress then (
      ...
      let ident = 2 + (String.get_uint8 buf ((P.byte_length * 2) - 1) land 1) in
  ...
  *)
    ...
    fun pk ->
      ...
      | Some x ->
          ...
          let sum = Fe.add sum b in
          (* y^2 *)
          let y = pow sum pident in
          (* https://tools.ietf.org/id/draft-jivsov-ecc-compact-00.xml#sqrt point 4.3*)
          let y' = Fe.sub p y in
          let y_str = Fe.to_octets (Fe.from_montgomery (T.out_y y)) in
          (* number must not be in montgomery domain*)
          let ident = String.get_uint8 pk 0 in
          let signY = 2 + (String.get_uint8 y_str 1 land 1) in
          let y = if Int.equal signY ident then y else y' in
          validate_finite_point_fe x y

  let of_octets buf =
    let len = P.byte_length in
    if String.length buf = 0 then Error `Invalid_format
    else
      match String.get_uint8 buf 0 with
      | 0x00 when String.length buf = 1 -> Ok (at_infinity ())
      | (0x02 | 0x03) when String.length pident > 0 -> decompress buf
...
module Make_point (P : Parameters) (F : Foreign_point) (Fe : Field_element) :
  Point =
  Make_point_base (P) (F) (Fe) (NoTransform)
...
module Make_point_twisted
  ...
  Make_point_base (P) (F) (Fe) (Twist (P) (Fe))
...
module Make_dh (Param : Parameters) (P : Point) (S : Scalar) : Dh = struct
  ...
  let secret_to_octets s = S.to_octets s
...
struct
  ...
  let padded msg =
    ...
    in
    if l > bl || (l = bl && not (first_byte_ok ())) then raise Message_too_long
    else if l = bl then msg
    else
  ...
  let sign ~key ?k msg =
    ...
    let rec do_sign g =
      ...
      let ksc =
        ...
        | Ok ksc -> ksc
        | Error _ -> invalid_arg "k not in range"
        (* if no k is provided, this cannot happen since K_gen_*.gen already preserves the Scalar invariants *)
      in
      ...
      | Some r ->
          ...
          if S.not_zero s && S.not_zero r then (r, s) else again ()
    in
    do_sign g
  ...
  let verify ~key (r, s) msg =
    try
      ...
      if not (S.is_in_range r && S.is_in_range s) then false
      else
        let msg = padded msg in
        ...
        match
          (S.of_octets (F.to_be_octets u1), S.of_octets (F.to_be_octets u2))
        with
        | Ok u1, Ok u2 -> (
            let point = P.scalar_mult_add u1 u2 key in
            match x_of_finite_point_mod_n point with
            | None -> false (* point is infinity *)
            | Some r' -> String.equal r r')
        | Error _, _ | _, Error _ -> false
    with Message_too_long -> false
end
