...
module Make_scalar_element_bip340 (P : Parameters) (F : Foreign_n_bip340) :
  Scalar_element_bip340 = struct
...
module Make_bip340
    (Param : Parameters)
    (F : Scalar_element_bip340)
    (P : Point)
    (S : Scalar) : Bip340 = struct
  ...
  let tagged_hash tag =
    ...
    fun msg -> H.feed_string ctx msg |> H.get |> H.to_raw_string
  ...
  let pub_of_octets bytes =
    if String.length bytes != Param.byte_length then Error `Invalid_length
    else Ok (Pub bytes)
  ...
  let sign ~key ?aux_rand msg =
    ...
    let k_sc' =
      Result.get_ok (S.of_octets (F.to_be_octets (F.from_montgomery k')))
    in
    (* Compute R = k'*G *)
    let r_point = P.scalar_mult_base k_sc' in
    (* Determine k based on Y coordinate of R *)
    let k = if has_even_y r_point then k' else F.opp k' in
    (* Extract r from R *)
  ...
  let verify ~key (r, s) msg =
    if String.length r != byte_length || String.length s != byte_length then
      false
    else
      ...
      | Ok s_sc -> (
          ...
          | Ok p_point -> (
              ...
              let e = if has_even_y p_point then F.opp e else e in
...
module P256k1 : P256k1 = struct
  module Params = struct
    ...
    let g_x =
      "\x79\xBE\x66\x7E\xF9\xDC\xBB\xAC\x55\xA0\x62\x95\xCE\x87\x0B\x07\x02\x9B\xFC\xDB\x2D\xCE\x28\xD9\x59\xF2\x81\x5B\x16\xF8\x17\x98"

    let g_y =
      "\x48\x3A\xDA\x77\x26\xA3\xC4\x65\x5D\xA4\xFB\xFC\x0E\x11\x08\xA8\xFD\x17\xB4\x48\xA6\x85\x54\x19\x9C\x47\xD0\x8F\xFB\x10\xD4\xB8"
    ...
    let pident =
      "\x3F\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xBF\xFF\xFF\x0C"
    (* (Params.p + 1) / 4*)
  ...
  module Foreign = struct
    ...
    external to_montgomery : out_field_element -> field_element -> unit
      = "mc_secp256k1_to_montgomery"
    [@@noalloc]

    external from_octets : out_field_element -> string -> unit
      = "mc_secp256k1_from_bytes"
    [@@noalloc]

    external set_one : out_field_element -> unit = "mc_secp256k1_set_one"
    [@@noalloc]
    ...
    external from_montgomery : out_field_element -> field_element -> unit
      = "mc_secp256k1_from_montgomery"
    [@@noalloc]

    external to_octets : bytes -> field_element -> unit
      = "mc_secp256k1_to_bytes"
    [@@noalloc]
  ...
  module Foreign_n = struct
    ...
    external opp : out_scalar_element -> scalar_element -> unit
      = "mc_nsecp256k1_opp"
    [@@noalloc]
