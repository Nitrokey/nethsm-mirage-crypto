...
module BrainpoolP256 : Dh_dsa  = struct
  ...
  module Params = struct
    ...
    let pident = "\x2A\x7E\xD5\xF6\xE8\x7B\xAA\x6F\x0F\x99\x82\xA4\x27\x60\xE3\x5C\x9B\x8E\xFD\x88\xF5\x49\x88\x0A\x08\x04\xD2\x07\x47\xDB\x94\xDE" (* (Params.p + 1) / 4*)
    let byte_length = 32
    let bit_length = 256
    let fe_length = 32
...
module BrainpoolP384 : Dh_dsa  = struct
  ...
  module Params = struct
    ...
    let pident = "\x23\x2E\x47\xA0\xA8\xCE\x1B\x4A\x03\xD7\x5B\xDF\x94\x39\x90\x77\xC5\x4B\xDC\x42\x7B\x55\x15\xAD\x04\xAC\x76\x86\x5F\xED\xC4\x48\xEB\x34\xE9\xCA\x64\x07\x46\x9C\x61\xD1\xC0\x04\xCC\x41\xFB\x15" (* (Params.p + 1) / 4*)
    let byte_length = 48
    let bit_length = 384
    let fe_length = 48
    let first_byte_bits = None
...
module BrainpoolP512 : Dh_dsa  = struct
  ...
  module Params = struct
    ...
    let pident = "" (* (Params.p + 1) / 4*)
    let byte_length = 64
    let bit_length = 512
    let fe_length = 64
    let first_byte_bits = None
